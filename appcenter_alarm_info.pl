...
sub get_statistic_data   # 获取告警信息
{
        ...
        qq { appname='oss_tas_appcenter_android_AppService' or appname='oss_tas_appcenter_android_CategoryService' or }.
        qq { appname='oss_tas_appcenter_android_CmsService' or appname='oss_tas_appcenter_android_tap_IOpenTapService' or }.
        qq { appname='sh_tas_appcenter_android_AppService' or appname='sh_tas_appcenter_android_CategoryService' or }.
        qq { appname='sh_tas_appcenter_android_CmsService' or appname='sh_tas_appcenter_android_tap_IOpenTapService' or }.
        #qq { (appname='ts_qqpim' and subappvalue like '\%SecureServer\%') or }
        qq { appname='oss_tas_appcenter_android_TopAppService' or appname='sh_tas_appcenter_android_TopAppService')as b}.
        qq { where   a.appname=b.appname  and a.f_date="$alarm_date" }; #通过appgroup"监控组"或appname"监控项"指定需要提取的告警
        printf("%s",$sql_data);
...
sub statistic_data_to_db
{
        ...
        my $table_name = "t_imdata_appcenter_alarm_stat"."_". $table_prefix; #告警数据入库表名,按天分表,alarm_stat
        my $stat_tm = `date -d "now" +"%H:%M"`; chomp $stat_tm;
...
sub alarm_info
{
    ...
    my $sql_data1;
    my $table_prefix  = `date -d "1 day ago" +"%Y%m%d"` ; chomp $table_prefix;
    my $table_name = "t_imdata_appcenter_alarm_stat"."_". $table_prefix;
    my $stat_name = "appcenter_alarm_stat";
...
sub alarm_info_to_db
{
        ...
        my $table_name = "t_imdata_appcenter_alarm_info"."_". $table_prefix; #监控信息入库表名,按天分表,alarm_info
        my $stat_name = "appcenter_alarm_info";
